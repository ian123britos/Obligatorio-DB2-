...
BEGIN
 SET NOCOUNT ON;
 IF NOT UPDATE(StsRepara)
 BEGIN
 RETURN;
 END
 INSERT INTO HistoricoReparacion(IdRepara, NumSerie, IdProd, EstadoAnterior, EstadoNuevo,FchCambio)
 SELECT i.IdRepara,i.NumSerie,i.IdProd,d.StsRepara,i.StsRepara,i.FchRepara
 FROM inserted i, deleted d 
 where i.IdRepara = d.IdRepara and i.StsRepara in ('Terminado'  , 'Cancelado');
END;
...
BEGIN
set nocount on;
if exists (select 1 from deleted d where d.StsRepara in ('En testing', 'Terminado'))
BEGIN
...
where StsRepara in ('Iniciado','Cancelado'))
END;

--la tabla deleted es la tupla anterior al cambio
--y la tabla iserted es la tupla despues del cambio
